...
if(isset($_POST["updatepro"]))
{
    ...
    if(isset($_FILES['profileimg']) && !empty($_FILES['profileimg']['name'])){
        ...
        if(in_array($img_ext,['png','jpeg','jpg']) === true){
            ...
            {
                $upsql = "update user set image = 'image/profileimg/$newimgname', name = '".trim($_POST["uname"])."' where uid = $_SESSION[uid]";
            }
        }
        else{
        ...
        $upsql = "update user set name = '".trim($_POST["uname"])."' where uid = $_SESSION[uid]";
    }
    if($upsql != ""){
